...
createdb_query = "CREATE DATABASE IF NOT EXISTS ibankingdb;"
table_query = """
    CREATE TABLE mysql-python-banking.account (
...
try:
    cnx = mysql.connector.connect(
        user='root', password='', host='localhost', database='ibankingdb')
    cursor = cnx.cursor()
...
class panelMain(wx.Panel):
    def __init__(self, parent):
        ...
        self.btnAcct = wx.Button(self, -1, "Create Account", (300, 100))
        self.btnBal = wx.Button(self, -1, "Check Balance", (150, 100))
        self.btnDep = wx.Button(self, -1, "Deposit", (150, 175))
        self.btnWdr = wx.Button(self, -1, "Withdraw", (300, 175))
        self.btnTfr = wx.Button(self, -1, "Transfer", (230, 225))
...
class panelAcct(wx.Panel):
    ...
    def onCreateAcctPress(self, event):
        if self.txtName.GetValue() != '' and self.txtOpenBal.GetValue() != '' and isfloat(self.txtOpenBal.GetValue()):
            ...
            addAcctQuery = "INSERT INTO ibankingdb.account (name_on_account, balance, account_open_date) VALUES (%s, %s, %s)"
...
class panelBal(wx.Panel):
    ...
    def onCheckBalPress(self, event):
        font = wx.Font(18, wx.DEFAULT, wx.NORMAL, wx.NORMAL)
        checkBalQuery = "SELECT * FROM ibankingdb.account WHERE account_no = " + \
            self.txtAcctNo.GetValue()
...
class panelDep(wx.Panel):
    ...
    def onShowDetailsPress(self, event):
        ...
        if self.acctNo.GetValue().isnumeric():
            ...
            rowLockQuery = "SELECT * FROM ibankingdb.account WHERE account_no = " + \
                self.acctNo.GetValue() + " FOR UPDATE"
            cursor.execute(rowLockQuery)
            row = cursor.fetchone()
            if row is not None:
                lblTxt = stringTheDeets(row)
...
class panelTfr(wx.Panel):
    ...
    def onShowDetailsPress(self, event):
        if (self.srcAcctNo.GetValue() == '') or (self.tgtAcctNo.GetValue() == ''):
            ...
            srcRowLockQuery = "SELECT * FROM ibankingdb.account WHERE account_no = " + \
                self.srcAcctNo.GetValue() + " FOR UPDATE"
            ...
            tgtRowLockQuery = "SELECT * FROM ibankingdb.account WHERE account_no = " + \
                self.tgtAcctNo.GetValue() + " FOR UPDATE"
...
class Program(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, wx.ID_ANY, 'ibankingdb')
